...
param.Amplitude = 2; %Amplitude for forced input

param.dtKalman = 0.0001; %EKF computation time step (discretisation)
...
% springmass = @(t,z) [z(2);(-param.k(1)*z(1) - param.k(2)*z(1)*z(1)*z(1) + param.Amplitude*sin(param.omega*t))/param.m];    
        %used as a reference for process model for the EKF
        [t,z] = ode45(springmass,tspan,z0);
...
P = zeros(n,n);%0.01*ones(n,n);%zeros(n,n);%2*eye(n); % NxN state covariance matrix
