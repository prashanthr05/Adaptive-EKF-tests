...
[f_B1_t, mu_B1_t, f_B2_t, mu_B2_t, omega_B] = rigidBodyInvDyn(Phi, dPhi, dPos, ddPos, tc, tDes, model, 0.5, 0.5);

%% Plots
if plots
    figure(1);
    subplot(2,1,1);
    plot(tc,f_B1_t); hold on;
    plot(tc,f_B2_t); axis tight; grid on;
    legend('{f^B}_1','{f^B}_2');
    xlabel('time t(sec)');
    ylabel('Force (N)');
    title('Inverse Dynamics');
    subplot(2,1,2);
    plot(tc,mu_B1_t); hold on;
    plot(tc,mu_B2_t); hold on;
    axis tight; grid on; 
