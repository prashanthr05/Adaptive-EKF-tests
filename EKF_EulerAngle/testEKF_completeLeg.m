...
for i = filterID
    ...
    clearvars -except filter filterID i setup
    close all
    
    [kalmanQParams,kalmanRParams,kIni,cIni] = setupCovariancesForExperiments(filter{i});
    ...
    for j = expID
        ...
        clearvars -except filter filterID i j expID measurementSuffix processSuffix n m p setup
        close all
        ...
        [yMeas,tMeas,model,RData] = realMeasurement_completeLeg(model.dtKalman,model,model.measurementPlots,t_min,t_max,measurementSuffix{j},7,'right','right');
        T = tMeas(end);
        tKalman = tMeas;
